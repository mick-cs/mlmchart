...
(function(factory) {
    ...
    function showChildren($node) {
        ...
        var $descendants = isVerticalDesc ? $temp.removeClass('hidden').find('.node:visible') : $temp.removeClass('hidden').eq(2).children().find('tr:first').find('.node:visible');
    ...
    function hideSiblings($node, direction) {
        ...
        $animatedNodes.eq(0).one('transitionend', function() {
            ...
            $siblings.find('.node:visible:gt(0)').removeClass('slide-left slide-right').addClass('slide-up').end().find('.lines, .nodes, .verticalNodes').addClass('hidden').end().addClass('hidden');
            if (isInAction($node)) {
    ...
    function createNode(nodeData, level, opts) {
        ...
        var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>').addClass('node ' + (nodeData.className || '') + (level >= opts.depth ? ' slide-up' : '')).append('<div class="title">' + nodeData[opts.nodeTitle] + '</div>').append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (nodeData[opts.nodeContent] || '') + '</div>' : '');
        ...
        if (opts.verticalDepth && (level + 2) > opts.verticalDepth) {
            ...
            if (Number(flags.substr(1, 1))) {
                $nodeDiv.append('<i class="edge horizontalEdge rightEdge fa"></i>' + '<i class="edge horizontalEdge leftEdge fa"></i>');
            }
                    ...
                    $.ajax({
                        ...
                        if ($node.closest('.orgchart').data('inAjax')) {
                            ...
                            if (data.children.length) {
                                addChildren($node, data, $.extend({}, opts, {
                                    depth: 0
                                }));
                            }
                        }
        ...
        if (hasChildren) {
            ...
            if (isVerticalLayer) {
                ...
                if (level + 2 === opts.verticalDepth) {
                    $nodeWrapper.append('<tr class="verticalNodes' + isHidden + '"><td></td></tr>').find('.verticalNodes').children().append($nodeLayer);
                } else {
    ...
    function buildParentNode($currentRoot, nodeData, opts, callback) {
        ...
        $.when(createNode(nodeData, 0, opts || $currentRoot.closest('.orgchart').data('options'))).done(function($nodeDiv) {
            ...
            $oc.prepend($table).children('table:first').append('<tr class="nodes"><td colspan="2"></td></tr>').children('tr:last').children().append($oc.children('table').last());
            callback();
        }).fail(function() {
        ...
        if ($nodeChart.parent().is('td')) {
            ...
            buildHierarchy($nodeChart.closest('.orgchart'), nodeData, 0, opts, function() {
                if (++nodeCount === siblingCount) {
                    complementLine($nodeChart.next().children('tr:last').children().eq(insertPostion).after($('<td colspan="2">').append($nodeChart)), siblingCount, 1);
